/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ClothingItemWhereInput } from "./ClothingItemWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ClothingItemOrderByInput } from "./ClothingItemOrderByInput";

@ArgsType()
class ClothingItemFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ClothingItemWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ClothingItemWhereInput, { nullable: true })
  @Type(() => ClothingItemWhereInput)
  where?: ClothingItemWhereInput;

  @ApiProperty({
    required: false,
    type: [ClothingItemOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ClothingItemOrderByInput], { nullable: true })
  @Type(() => ClothingItemOrderByInput)
  orderBy?: Array<ClothingItemOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ClothingItemFindManyArgs as ClothingItemFindManyArgs };
